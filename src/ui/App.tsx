...
const AGILogo: React.FC = () => {
  ...
  const gradientColors = ["cyan", "magenta", "blue", "green", "yellow"] as const;
  ...
  return (
    <Box flexDirection="column" marginBottom={1}>
      {lines.map((line, lineIndex) => (
        <Box key={lineIndex}>
          {line.split("").map((char, charIndex) => (
            <Text
              key={charIndex}
              color={gradientColors[charIndex % gradientColors.length]}
              bold
            >
              {char}
            </Text>
      ...
      <Box marginTop={1}>
        <Text dimColor>
          AGI = "Actually Good Intern" at your service.
        </Text>
      </Box>
    </Box>
  );
